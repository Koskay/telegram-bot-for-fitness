...
async def sub_categories_kb(categories_id, save, user_id):
    curr_level = 1
    markup = InlineKeyboardMarkup(row_width=1)

    sub_categories = await db.get_sub_categories(categories_id, user_id)
    for category in sub_categories:
        butt_text = f'{category[2]}'
        data = make_callback_data(level=curr_level+1, category=category[1], exercises=category[3], save=save)
